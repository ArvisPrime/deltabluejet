 ...
 */
function getFlightDurationMs(flight: FirebaseFirestore.DocumentData): number {
  const depMs = flight.departureTime?.toMillis?.() || 0;
  const arrMs = flight.arrivalTime?.toMillis?.() || 0;
  if (arrMs > depMs) return arrMs - depMs;
...
export const checkFlightStatus = onSchedule(
  ...
  async () => {
    ...
    const snap = await db.collection('flights')
      .where('departureTime', '>=', Timestamp.fromDate(startWindow))
      .where('departureTime', '<=', Timestamp.fromDate(endWindow))
      .get();
    ...
    for (const doc of snap.docs) {
      ...
      if (newStatus && newStatus !== status) {
        ...
        if (newStatus === 'taxi_out' || newStatus === 'departed') {
          updateData.actualDepartureTime = FieldValue.serverTimestamp();
        }
        if (newStatus === 'airborne') {
          updateData.actualTakeoffTime = FieldValue.serverTimestamp();
        }
        if (newStatus === 'landed') {
          updateData.actualLandingTime = FieldValue.serverTimestamp();
        }
