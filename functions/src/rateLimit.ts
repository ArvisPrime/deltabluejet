...
interface RateLimitConfig {
    ...
    windowMs?: number;
    /** Firestore collection to store counters */
    collection?: string;
}
...
export async function enforceRateLimit(
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (!snap.exists) {
            ...
            transaction.set(docRef, {
                userId,
                action,
                requests: [{ timestamp: FieldValue.serverTimestamp() }],
                count: 1,
        ...
        transaction.update(docRef, {
            requests: [...recentRequests, { timestamp: FieldValue.serverTimestamp() }],
    ...
    if (!result.allowed) {
        ...
        throw new HttpsError(
            'resource-exhausted',
            `Rate limit exceeded for ${action}. Maximum ${maxRequests} requests per ${retryAfterMinutes} minutes. Please try again later.`,
        );
    }
}
