...
async function seed() {
    ...
    // --- Seed CMS Config ---
    console.log('📄 Seeding CMS config (header, footer, menus)...');

    // Header config
    await db.doc('cmsConfig/header').set({
        logoUrl: null,
        announcementBar: '✈️ New routes to London & Dubai now available — Book early for the best fares!',
        announcementActive: true,
        navItems: [
            { label: 'Book', href: '/book', order: 0, children: [] },
            { label: 'Check-in', href: '/check-in', order: 1, children: [] },
            { label: 'My Trips', href: '/manage-booking', order: 2, children: [] },
            { label: 'Flight Status', href: '/flight-status', order: 3, children: [] },
            { label: 'Destinations', href: '/destinations', order: 4, children: [] },
        ],
        updatedAt: Timestamp.now(),
    });

    // Footer config
    await db.doc('cmsConfig/footer').set({
        columns: [
            {
                title: 'Company',
                links: [
        ...
        socialLinks: [
            { platform: 'facebook', url: 'https://facebook.com/deltabluejet' },
            { platform: 'twitter / x', url: 'https://x.com/deltabluejet' },
            { platform: 'instagram', url: 'https://instagram.com/deltabluejet' },
            { platform: 'linkedin', url: 'https://linkedin.com/company/deltabluejet' },
        ],
        contactEmail: 'info@deltablue.to',
        contactPhone: '+220-422-8800',
        copyrightText: '© 2026 Deltablue Jet Air. All rights reserved. Use of this site indicates compliance with our Privacy Policy and Terms of Carriage.',
        updatedAt: Timestamp.now(),
    });

    // Menu config
    await db.doc('cmsConfig/menus').set({
        items: [
            {
                label: 'Book', href: '/book', order: 0, children: [
                    { label: 'Search Flights', href: '/book/search', order: 0, children: [] },
                    { label: 'Multi-city', href: '/book/multi-city', order: 1, children: [] },
                    { label: 'Group Booking', href: '/book/group', order: 2, children: [] },
                ]
            },
            { label: 'Check-in', href: '/check-in', order: 1, children: [] },
            { label: 'My Trips', href: '/manage-booking', order: 2, children: [] },
            { label: 'Destinations', href: '/destinations', order: 3, children: [] },
            { label: 'Loyalty Club', href: '/loyalty', order: 4, children: [] },
        ],
        updatedAt: Timestamp.now(),
    });

    console.log('   → cmsConfig/header, footer, menus created');

    // --- Seed CMS Pages ---
    console.log('📝 Seeding CMS pages...');
    const cmsPages = [
        {
            title: 'About Deltablue Jet Air',
            slug: 'about',
            content: '<h2>Our Story</h2><p>Founded in Banjul, The Gambia, Deltablue Jet Air connects West Africa to the world with reliability, comfort, and African hospitality.</p><h3>Our Mission</h3><p>To be the premier airline connecting The Gambia and West Africa to key global destinations.</p>',
            metaTitle: 'About Us | Deltablue Jet Air',
            metaDescription: 'Learn about Deltablue Jet Air, connecting West Africa to the world.',
            featuredImage: null,
            parentPage: null,
            tags: ['company', 'about'],
            status: 'published',
            author: 'admin@deltablue.to',
        },
        {
            title: 'Terms & Conditions',
            slug: 'terms',
            content: '<h2>Terms of Carriage</h2><p>These terms govern your travel with Deltablue Jet Air. Please read carefully before booking.</p>',
            metaTitle: 'Terms & Conditions | Deltablue Jet Air',
            metaDescription: 'Read the terms and conditions for flying with Deltablue Jet Air.',
            featuredImage: null,
            parentPage: null,
            tags: ['legal'],
            status: 'published',
        ...
        },
        {
            ...
            slug: 'privacy',
            content: '<h2>Privacy Policy</h2><p>Your privacy is important to us. This policy describes how we collect, use, and protect your personal information.</p>',
            metaTitle: 'Privacy Policy | Deltablue Jet Air',
            metaDescription: 'Privacy policy for Deltablue Jet Air passengers and website visitors.',
            featuredImage: null,
            parentPage: null,
            tags: ['legal', 'privacy'],
            status: 'published',
        ...
        },
        {
            ...
            slug: 'baggage',
            content: '<h2>Baggage Information</h2><p>Economy: 23kg checked + 7kg carry-on. Business: 32kg checked + 10kg carry-on. First: 2x32kg checked + 10kg carry-on.</p>',
            metaTitle: 'Baggage Info | Deltablue Jet Air',
            metaDescription: 'Baggage allowance and fees for Deltablue Jet Air flights.',
            featuredImage: null,
            parentPage: null,
            tags: ['travel', 'baggage'],
            status: 'draft',
    ...
    const auditEntries = [
        { action: 'Template Edit: Flight Delay', entityType: 'emailTemplate', entityId: 'tpl-delay-01', userId: 'usr-001', userEmail: 'chen.m@deltablue.com', details: { field: 'subject', oldValue: 'Delay Notice', newValue: 'Important: Flight Delay Update' }, timestamp: pastDate(0) },
        { action: 'Gate Move: A12 to C04', entityType: 'flight', entityId: 'fl-101', userId: 'usr-002', userEmail: 'rivera.a@deltablue.com', details: { oldGate: 'A12', newGate: 'C04', flightNumber: 'DB-101' }, timestamp: pastDate(0) },
        { action: 'Emergency Maintenance Flag', entityType: 'aircraft', entityId: 'ac-006', userId: 'usr-003', userEmail: 'connor.s@deltablue.com', details: { reason: 'Engine inspection required', registration: 'DB-7382' }, timestamp: pastDate(0) },
        { action: 'Automated Manifest Sync', entityType: 'system', entityId: 'sys-manifest', userId: 'system', userEmail: 'ops.auto@deltablue.com', details: { flightsSynced: 24, status: 'verified' }, timestamp: pastDate(0) },
        { action: 'Pricing Override Applied', entityType: 'flight', entityId: 'fl-115', userId: 'usr-004', userEmail: 'alex.m@deltablue.com', details: { fareClass: 'business', oldPrice: 1200, newPrice: 980, reason: 'Loyalty discount' }, timestamp: pastDate(0) },
        { action: 'New Route Created: BJL-IST', entityType: 'route', entityId: 'bjl-ist', userId: 'usr-001', userEmail: 'chen.m@deltablue.com', details: { origin: 'BJL', destination: 'IST', status: 'active' }, timestamp: pastDate(1) },
        { action: 'User Role Update', entityType: 'user', entityId: 'usr-003', userId: 'usr-001', userEmail: 'chen.m@deltablue.com', details: { oldRole: 'cs_agent', newRole: 'ops_manager' }, timestamp: pastDate(1) },
        { action: 'Flight Cancellation: DB-142', entityType: 'flight', entityId: 'fl-142', userId: 'usr-002', userEmail: 'rivera.a@deltablue.com', details: { reason: 'Weather conditions', passengersAffected: 156 }, timestamp: pastDate(2) },
    ];

    const batch7 = db.batch();
    for (const entry of auditEntries) {
        const ref = db.collection('auditLogs').doc();
        batch7.set(ref, entry);
    }
    await batch7.commit();
    ...
    console.log('   • cmsConfig (header, footer, menus)');
    console.log(`   • cmsPages (${cmsPages.length})`);
    console.log(`   • auditLogs (${auditEntries.length})`);
}
