...
const FlightTrackerSearch: React.FC = () => {
  const action = useAdminAction();
   const navigate = useNavigate();
   const onNext = () => navigate(ROUTES.FLIGHT_TRACKER_RESULTS);
         ...
         <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div className="space-y-10">
               <div className="space-y-6">
                  ...
                  <h1 className="text-6xl font-black text-navy-950 tracking-tighter uppercase leading-[0.9]">Track Live <br />Flights</h1>
                  <p className="text-navy-500 font-medium italic text-xl leading-relaxed uppercase tracking-wider opacity-80">
                     Monitor real-time positioning, gate assignments, and terminal logic nodes for any {BRAND.shortName} flight.
                  </p>
               </div>
               ...
               <div className="bg-primary/5 p-8 rounded-[3rem] border border-primary/10 flex gap-6 items-start shadow-inner">
                  ...
                  <div className="space-y-1">
                     <p className="text-sm font-black text-navy-950 uppercase tracking-tight">Live Tracking</p>
                     <p className="text-[10px] font-bold text-navy-400 uppercase tracking-widest leading-relaxed italic opacity-70">
                        Transponder signals are updated every 12 seconds via satellite link.
                     </p>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-[4.5rem] border border-navy-100 p-12 md:p-16 shadow-[0_60px_120px_-30px_rgba(0,0,0,0.15)] relative group overflow-hidden">
               ...
               <div className="flex bg-navy-50 p-1.5 rounded-[1.75rem] shadow-inner mb-12">
                  <button
                     ...
                     className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${searchMode === 'ID' ? 'bg-white text-navy-950 shadow-lg border border-navy-100' : 'text-navy-400 hover:text-navy-700'
                        }`}
                  >
                     Flight Number
                  </button>
                  <button
                     ...
                     className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${searchMode === 'ROUTE' ? 'bg-white text-navy-950 shadow-lg border border-navy-100' : 'text-navy-400 hover:text-navy-700'
                        }`}
                  >
                     Route Search
                  </button>
               ...
               <form className="space-y-12" onSubmit={(e) => { e.preventDefault(); onNext(); }}>
                  {searchMode === 'ID' ? (
                     <div className="space-y-4 animate-in slide-in-from-left-4 duration-300">
                        <label className="text-[10px] font-black text-navy-400 uppercase tracking-[0.3em] block px-2">Flight Number</label>
                     ...
                     <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                        <div className="space-y-4">
                           <label className="text-[10px] font-black text-navy-400 uppercase tracking-[0.3em] block px-2">Origin Airport</label>
                  ...
                  <div className="pt-6 space-y-6">
                     <button onClick={action('Track Flight my_location — action triggered', 'info')} className="w-full h-20 bg-primary text-white font-black uppercase tracking-[0.3em] text-xs rounded-[2rem] shadow-2xl shadow-primary/30 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-6 group/btn">
                        Track Flight <span className="material-symbols-outlined text-2xl group-hover/btn:translate-x-2 transition-transform">my_location</span>
                     </button>
