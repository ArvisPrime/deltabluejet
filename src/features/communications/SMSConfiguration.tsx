...
import type { SmsTemplateDoc, TemplateStatus } from '../../types/firestore';
...
const SMSConfiguration: React.FC = () => {
   ...
   const onOpenLog = () => navigate(ROUTES.SMS_AUDIT_LOG);

   const [templates, setTemplates] = useState<SmsTemplateDoc[]>([]);
   ...
   const loadTemplates = useCallback(async () => {
      ...
      try {
         ...
         useToastStore.getState().addToast("Failed to load SMS templates", "error");
      } finally {
         setLoading(false);
      }
   }, [selectedId]);
   ...
   const handleSelect = (t: SmsTemplateDoc) => {
      setSelectedId(t.id);
      setMessage(t.body || '');
   };
   ...
   const handleSave = async () => {
      ...
      setSaving(true);
      try {
         await updateSmsTemplate(selectedId, { body: message });
         await loadTemplates();
      } catch (err) {
         console.error('Failed to save SMS template:', err);
         useToastStore.getState().addToast("Failed to save SMS template", "error");
      } finally {
         setSaving(false);
      }
   };
   ...
   const handleNewTemplate = async () => {
      try {
         ...
         useToastStore.getState().addToast("Failed to create SMS template", "error");
      }
   };
   ...
   const statusLabel = (s: TemplateStatus) => s === 'live' ? 'Active' : s === 'draft' ? 'Draft' : 'Inactive';
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
                     {loading ? (
                        ...
                        <div className="text-center py-20 space-y-4">
                           ...
                           <p className="text-xs font-black text-navy-300 uppercase tracking-widest">No SMS templates yet</p>
                        </div>
                     ) : (
                        templates.map((t) => (
                           <button
                              ...
                              className={`w-full p-5 rounded-2xl flex flex-col gap-1 text-left transition-all border-2 ${selectedId === t.id ? 'bg-primary/5 border-primary shadow-lg shadow-primary/5' : 'bg-white border-transparent hover:bg-navy-50'
                                 }`}
                           >
                              <div className="flex justify-between items-center mb-1">
                                 ...
                                 <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border ${t.status === 'live' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                    t.status === 'draft' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-navy-50 text-navy-400 border-navy-100'
                                    }`}>{statusLabel(t.status)}</span>
                              </div>
                              <p className="text-[10px] font-medium text-navy-400 line-clamp-1 italic">{t.body?.slice(0, 60)}...</p>
                           </button>
                        ))
                     )}
                  </div>
                  <div className="p-6 border-t border-navy-50 bg-navy-50/20">
                     <button onClick={handleNewTemplate} className="w-full py-4 rounded-2xl bg-white border-2 border-dashed border-navy-100 text-[10px] font-black uppercase tracking-widest text-navy-400 hover:border-primary hover:text-primary transition-all flex items-center justify-center gap-3">
               ...
               <div className="flex-1 flex flex-col gap-8 overflow-y-auto custom-scrollbar pr-2 pb-12">
                  ...
                  <div className="bg-white rounded-[3rem] border border-navy-100 p-8 shadow-sm space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-3">
                           ...
                           <input className="w-full h-14 px-8 bg-navy-50 border-none rounded-2xl text-sm font-black text-navy-950 focus:ring-8 focus:ring-primary/5 transition-all shadow-inner" defaultValue={selected?.name || ''} />
                        </div>
                        <div className="space-y-3">
                           <label className="text-[10px] font-black text-navy-400 uppercase tracking-widest block px-1">Trigger Event</label>
                           <div className="relative group">
                              <select className="w-full h-14 px-8 bg-navy-50 border-none rounded-2xl text-xs font-black text-navy-950 uppercase appearance-none focus:ring-8 focus:ring-primary/5 transition-all shadow-inner">
                                 ...
                                 <option>Manual Staff Send</option>
                              </select>
                     ...
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                           <div className="flex items-center gap-3">
                              ...
                              <span className="px-2 py-0.5 rounded bg-navy-50 text-navy-500 text-[8px] font-black uppercase tracking-widest border border-navy-100">v{selected ? '1.0' : '--'}</span>
                           </div>
                           <p className="text-[10px] font-bold text-navy-400 uppercase tracking-widest mt-1 opacity-60">Provider: <span className="text-navy-950">{selected?.provider || 'N/A'}</span> • Category: {selected?.category || '--'}</p>
                        </div>
                     </div>
                     <button onClick={onOpenLog} className="flex items-center gap-2 px-6 py-2.5 rounded-xl border border-navy-100 text-[10px] font-black uppercase text-navy-700 hover:text-primary transition-all">
                  ...
                  <div className="bg-white rounded-[3.5rem] border border-navy-100 p-10 shadow-sm flex-1 flex flex-col space-y-8">
                     <div className="flex justify-between items-center border-b border-navy-50 pb-6">
                        <h3 className="text-xl font-black text-navy-950 uppercase tracking-tight">SMS Message Body</h3>
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                           {(selected?.variables || ['flight_number', 'destination', 'new_time', 'passenger_name', 'gate_id']).map(v => (
                              <button
                                 key={v}
                           ...
                           <button onClick={action('Opening extended SMS options…', 'info')} className="px-4 py-2 bg-navy-50 text-navy-400 border border-navy-100 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-navy-100 transition-all">More...</button>
                        </div>
                     </div>
                     ...
                     <div className="relative flex-1 group">
                        ...
                        <div className="absolute bottom-6 right-10 flex gap-6 text-[10px] font-black text-navy-300 uppercase tracking-[0.2em] select-none pointer-events-none">
                           ...
                           <span>{segments} Message Segment{segments > 1 ? 's' : ''}</span>
                        </div>
                     </div>

                     <div className="pt-6 border-t border-navy-50 flex justify-between items-center">
                        <button onClick={action('Sending test SMS…', 'success')} className="text-[10px] font-black text-navy-400 uppercase tracking-widest hover:text-red-500 transition-colors">Revert to Master Default</button>
                        <div className="flex gap-4">
                           <button onClick={action('Sending test SMS…', 'success')} className="px-8 py-3.5 bg-white border-2 border-navy-100 text-navy-700 font-black uppercase text-[10px] tracking-widest rounded-2xl hover:bg-navy-50 transition-all shadow-sm">Send Test SMS</button>
                           <button onClick={handleSave} disabled={saving} className="px-10 py-3.5 bg-primary text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl shadow-primary/30 hover:scale-105 active:scale-95 transition-all disabled:opacity-50">
               ...
               <div className="hidden xl:flex w-80 lg:w-96 flex-col items-center justify-center bg-navy-50/50 rounded-[4rem] border border-navy-100 p-12 shrink-0 relative overflow-hidden group">
                  ...
                  <div className="relative w-full max-w-[300px] h-[600px] bg-navy-950 rounded-[3.5rem] border-[8px] border-navy-900 shadow-2xl overflow-hidden flex flex-col">
                     ...
                     <div className="px-6 py-4 bg-navy-900/50 backdrop-blur-md border-b border-white/5 flex items-center gap-4">
                        ...
                        <div>
                           <p className="text-xs font-black text-white uppercase tracking-tight">{BRAND.shortName} Info</p>
                           <p className="text-[8px] font-bold text-white/30 uppercase tracking-[0.2em]">Live Connection</p>
                        </div>
                     </div>
                     ...
                     <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 custom-scrollbar">
                        ...
                        <div className="self-start max-w-[85%] animate-pulse">
                           <div className="bg-primary text-white rounded-3xl rounded-tl-none p-5 text-[11px] font-black leading-relaxed shadow-xl shadow-primary/20 relative">
                              ...
                              {message.replace(/{{(\w+)}}/g, (_match: string, p1: string) => {
                                 switch (p1) {
                           ...
                           <p className="text-[8px] font-black text-primary uppercase tracking-[0.3em] mt-3 ml-2">Live Preview</p>
                        </div>
                     </div>
