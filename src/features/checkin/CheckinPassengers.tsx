...
const CheckinPassengers: React.FC = () => {
   const navigate = useNavigate();
   const onNext = () => navigate(ROUTES.CHECKIN_DECLARATION);
   const onBack = () => navigate(ROUTES.CHECKIN);
   return (
      <div className="p-8 md:p-20 max-w-5xl mx-auto space-y-12 animate-in slide-in-from-right duration-700 font-display">
         <div className="space-y-4">
            <div className="flex justify-between items-end border-b border-navy-100 pb-8">
               <div className="space-y-2">
                  <h1 className="text-4xl font-black text-navy-950 tracking-tighter uppercase leading-none">Passenger Verification</h1>
                  <p className="text-navy-500 font-medium italic text-lg opacity-80 uppercase tracking-widest">Select the travelers associated with Booking DJXJ799.</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-black text-navy-300 uppercase tracking-widest mb-1">Flight</p>
                  ...
                  <div className="flex items-center gap-8">
                     ...
                     <div className="flex items-center gap-6">
                        ...
                        <div>
                           ...
                           <p className="text-[10px] font-bold text-navy-400 uppercase tracking-widest italic opacity-60">Passenger ID: {p.id}</p>
                        </div>
                     </div>
                  </div>
                  <div className="flex items-center gap-6">
                     {p.primary && <span className="bg-primary/10 text-primary text-[8px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border border-primary/20">Lead Passenger</span>}
         ...
         <div className="pt-10 flex flex-col md:flex-row items-center justify-between gap-8 border-t border-navy-50">
            ...
            <button onClick={onNext} className="px-14 py-6 bg-primary text-white font-black uppercase tracking-[0.25em] text-xs rounded-[2rem] shadow-2xl shadow-primary/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-4">
               Verify Passengers <span className="material-symbols-outlined text-xl">verified_user</span>
            </button>
         </div>
      </div>
   );
