         ...
         <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="space-y-2">
               <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-navy-300">
                  <button onClick={() => navigate(ROUTES.EXPERIMENTS_DASHBOARD)} className="hover:text-primary transition-all">Optimization Terminal</button>
            ...
            {[
               ...
               <div key={i} className="bg-white p-8 rounded-[3rem] border border-navy-100 shadow-sm flex flex-col group hover:shadow-xl transition-all relative overflow-hidden">
                  ...
                  <div className="flex items-baseline gap-4 relative z-10">
                     ...
                     {s.sub && (
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border shadow-sm ${s.sub.startsWith('+') ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-navy-50 text-navy-400 border-navy-100'
                           }`}>
                           {s.sub}
                        </span>
                     )}
