...
import { collection, getDocs, query, orderBy, limit, startAfter, doc, updateDoc, serverTimestamp, addDoc, type DocumentSnapshot } from 'firebase/firestore';
...
const UserManagement: React.FC = () => {
  ...
  const [newUserForm, setNewUserForm] = useState({ displayName: '', email: '', role: 'customer' as string });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      active: users.length,
      mfa: mfaCount,
      mfaPercent: users.length > 0 ? Math.round((mfaCount / users.length) * 100) : 0,
    };
  }, [users, totalCount]);
  ...
  const handleCreateUser = useCallback(async () => {
    ...
    try {
      ...
      setNewUserForm({ displayName: '', email: '', role: 'customer' });
    ...
    <div className="p-4 md:p-8 space-y-8 md:space-y-10 animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Loaded on Page', val: filteredUsers.length.toString(), icon: 'visibility', col: 'text-emerald-600' },
          { label: 'Two-Factor Enabled', val: stats.mfa.toString(), icon: 'security', col: 'text-indigo-600' },
          { label: 'Two-Factor Rate', val: `${stats.mfaPercent}%`, icon: 'verified_user', col: 'text-amber-600' },
        ].map((stat, i) => (
        ...
        <div className="p-4 md:p-8 border-b border-navy-100 bg-navy-50/10 flex flex-col xl:flex-row gap-6 justify-between items-center">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4 w-full lg:w-auto">
            <select
              ...
              className="flex-1 sm:flex-none h-12 md:h-14 px-4 md:px-8 bg-white border-2 border-navy-100 rounded-2xl text-[10px] font-black uppercase tracking-widest text-navy-900 appearance-none cursor-pointer"
            >
            ...
            <table className="w-full text-left min-w-[800px]">
              <thead>
                <tr className="bg-navy-50/50 border-b border-navy-50 text-[10px] font-black text-navy-300 uppercase tracking-[0.25em]">
                  ...
                  <th className="px-6 md:px-10 py-6">Two-Factor</th>
                  <th className="px-6 md:px-10 py-6">Last Login</th>
                  <th className="px-6 md:px-10 py-6 text-right">Details</th>
                </tr>
              </thead>
