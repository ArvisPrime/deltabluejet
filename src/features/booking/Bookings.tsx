        ...
        <div className="border-b border-navy-100 px-8 flex items-center gap-8 overflow-x-auto">
          {['All', 'Confirmed', 'Pending', 'Canceled', 'Groups'].map((tab) => (
            <button
              ...
              className={`py-5 text-sm font-black tracking-tight transition-all relative ${activeTab === tab ? 'text-primary' : 'text-navy-400 hover:text-navy-600'
                }`}
            >
              {tab} Bookings
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-navy-50">
              {mockBookings.map((b) => (
                <tr key={b.id} className="group hover:bg-navy-50/50 transition-colors cursor-pointer" onClick={() => navigate(ROUTES.MANAGE_BOOKING + '/detail')}>
                  ...
                  <td className="px-8 py-5">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${b.status === 'Confirmed' || b.status === 'Paid'
