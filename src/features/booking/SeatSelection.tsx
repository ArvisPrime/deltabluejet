...
const SeatSelection: React.FC = () => {
   ...
   const onNext = () => navigate(ROUTES.PAYMENT);
         ...
         <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-between items-end border-b border-navy-100 pb-8">
               ...
               <div className="flex flex-col items-end">
                  <span className="text-xs font-black text-navy-950 uppercase tracking-tight bg-navy-50 px-4 py-1.5 rounded-full border border-navy-100">Flight DJ-145</span>
               </div>
            </div>
         </div>
                           ...
                           return (
                              <div key={row} className="relative flex justify-between items-center group">
                                 ...
                                 <button
                                    ...
                                    className={`size-12 rounded-xl flex items-center justify-center text-[10px] font-black transition-all border-2 shadow-sm ${selectedSeat === `${row}A` ? 'bg-primary text-white border-primary scale-110 shadow-lg ring-4 ring-primary/20' :
                                 ...
                                 <div className="flex gap-4">
                                    {['B', 'C'].map(col => (
                                       <button
                                          ...
                                          className={`size-12 rounded-xl flex items-center justify-center text-[10px] font-black transition-all border-2 shadow-sm ${selectedSeat === `${row}${col}` ? 'bg-primary text-white border-primary scale-110 shadow-lg ring-4 ring-primary/20' :
               ...
               <div className="bg-white rounded-[3rem] border border-navy-100 shadow-2xl overflow-hidden sticky top-8">
                  <div className="flex border-b border-navy-100">
                     <button
                        ...
                        className={`flex-1 py-6 font-black text-[10px] uppercase tracking-widest transition-all ${activePassenger === 1 ? 'text-primary border-b-4 border-primary bg-primary/5' : 'text-navy-400 hover:bg-navy-50'
                           }`}
                     >
                        Passenger 1
                     </button>
                  ...
                  <div className="p-10 space-y-10">
                     <div className="flex items-center gap-6">
                        ...
                        <div>
                           ...
                           <p className="text-[10px] font-bold text-navy-400 uppercase tracking-widest mt-2 opacity-60">Adult • Booking #882-XJ</p>
                        </div>
                     </div>
                     ...
                     <div className={`bg-navy-950 p-8 rounded-[2.5rem] space-y-6 relative overflow-hidden shadow-2xl transition-all duration-700 ${selectedSeat ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-50'}`}>
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                           <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] font-black text-white/60 uppercase tracking-widest flex items-center gap-2">
                              <span className="material-symbols-outlined text-xs">window</span> {selectedSeat?.includes('A') || selectedSeat?.includes('C') ? 'Window Seat' : 'Aisle Access'}
                           </span>
                     ...
                     <div className="space-y-6 pt-6 border-t border-navy-50">
                        ...
                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                           <span className="text-primary">Seat Selection Fee</span>
                           <span className="text-emerald-600">{selectedSeat?.startsWith('9') ? '+$45.00' : 'Free'}</span>
                        </div>
                        <div className="pt-8 border-t-2 border-dashed border-navy-50 flex justify-between items-end">
                           <div className="space-y-1">
                              <span className="text-[10px] font-black text-navy-300 uppercase tracking-widest">Total Price</span>
                     ...
                     <div className="flex flex-col gap-4">
                        <button
                           ...
                           onClick={onNext}
                           className={`w-full py-6 rounded-[1.75rem] font-black uppercase tracking-[0.25em] text-sm transition-all shadow-2xl flex items-center justify-center gap-4 ${selectedSeat
