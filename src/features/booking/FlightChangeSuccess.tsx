...
const FlightChangeSuccess: React.FC = () => {
   ...
   return (
      <div className="p-8 max-w-5xl mx-auto space-y-12 animate-in slide-in-from-bottom duration-700 font-display pb-32">
         <div className="text-center space-y-6 flex flex-col items-center py-10">
            ...
            <div className="space-y-4">
               ...
               <p className="text-navy-400 font-bold text-lg max-w-2xl mx-auto italic uppercase tracking-wider leading-relaxed opacity-80 px-4">
                  Flight successfully changed. A confirmation digest has been sent to your email at <span className="text-navy-950 underline">{BRAND.placeholderEmail}</span>. Your previous booking has been cancelled.
               </p>
            </div>
         </div>
         ...
         <div className="bg-white rounded-[4rem] border border-navy-100 shadow-2xl overflow-hidden group">
            <div className="p-12 bg-navy-50/50 flex flex-col md:flex-row gap-10 justify-between items-center border-b border-navy-50">
               <div className="space-y-2 text-center md:text-left">
                  <p className="text-[10px] font-black text-navy-400 uppercase tracking-[0.3em]">Booking Reference</p>
                  <div className="flex items-center gap-6">
                     <span className="text-6xl font-black text-primary tracking-tighter uppercase">LGW882</span>
                     <button onClick={() => { navigator.clipboard.writeText('LGW882'); }} className="text-navy-200 hover:text-primary p-2 bg-white rounded-xl shadow-sm border border-navy-50 transition-all hover:scale-105 active:scale-95"><span className="material-symbols-outlined text-2xl">content_copy</span></button>
                  </div>
               </div>
            ...
            <div className="p-12 space-y-12">
               <div className="flex items-center justify-between border-b border-navy-50 pb-8">
                  ...
                  <span className="text-[10px] font-black text-navy-400 uppercase tracking-widest">Flight: DJ-102</span>
               </div>
               ...
               <div className="flex flex-col md:flex-row gap-12 items-center bg-navy-50/30 p-12 rounded-[3.5rem] border border-navy-100 relative">
                  <div className="flex flex-col items-center md:items-end min-w-[140px] text-center md:text-right space-y-2">
                     ...
                     <span className="text-[10px] font-bold text-navy-300 uppercase tracking-widest">London Heathrow</span>
                  </div>
                  <div className="flex-1 flex flex-col items-center px-10 relative">
                     <span className="text-[9px] font-black text-navy-200 uppercase tracking-[0.3em] mb-4">7h 50m • Direct Flight</span>
               ...
               <div className="grid grid-cols-1 md:grid-cols-4 gap-12 pt-8 border-t border-navy-50">
                  {[
                     { lbl: 'Flight', val: 'DJ 117' },
                     { lbl: 'Cabin Class', val: 'Economy Standard' },
                     { lbl: 'Aircraft', val: 'Embraer ERJ-120' },
                     { lbl: 'Seat', val: 'Auto-assigned' }
                  ].map((m, i) => (
            ...
            <div className="bg-navy-950 p-10 flex flex-col md:flex-row items-center justify-between gap-10">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.25em] mb-1">Payment Status</p>
                     <h4 className="text-2xl font-black text-white uppercase tracking-tighter">No Additional Charge</h4>
                  </div>
               </div>
               <div className="text-center md:text-right">
                  <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em] mb-1">Payment Settled</p>
         ...
         <div className="flex flex-col sm:flex-row items-center justify-center gap-8 pt-8">
            <button onClick={() => { const html = `<!DOCTYPE html><html><head><title>Booking Confirmation LGW882</title><style>body{font-family:Arial,sans-serif;max-width:600px;margin:40px auto;padding:20px}h1{font-size:24px}table{width:100%;border-collapse:collapse;margin:20px 0}td,th{padding:8px 12px;text-align:left;border-bottom:1px solid #eee;font-size:13px}th{font-size:10px;text-transform:uppercase;color:#999}</style></head><body><h1>Booking Confirmation</h1><p><strong>Reference:</strong> LGW882</p><p><strong>Flight:</strong> DJ-102 &bull; LHR &rarr; JFK</p><table><tr><th>Detail</th><th>Value</th></tr><tr><td>Cabin Class</td><td>Economy Standard</td></tr><tr><td>Aircraft</td><td>Embraer ERJ-120</td></tr><tr><td>Seat</td><td>Auto-assigned</td></tr><tr><td>Payment</td><td>$0.00 Exchange</td></tr></table></body></html>`; const blob = new Blob([html], { type: "text/html" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "Confirmation-LGW882.html"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }} className="px-10 py-4 bg-white border-2 border-navy-100 text-navy-700 font-black uppercase text-[10px] tracking-widest rounded-2xl hover:bg-navy-50 shadow-sm flex items-center gap-3 transition-all">
               <span className="material-symbols-outlined text-xl">download</span> Download Confirmation
            </button>
            <button
            ...
            >
               Manage Reservation <span className="material-symbols-outlined text-xl">arrow_forward</span>
            </button>
         </div>

         <div className="flex justify-center pt-8">
            <button onClick={() => navigate(ROUTES.DASHBOARD)} className="text-[10px] font-black text-navy-300 uppercase tracking-[0.3em] hover:text-primary transition-all flex items-center gap-2 group">
